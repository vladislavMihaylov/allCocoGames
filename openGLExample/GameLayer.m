...
-(void)showAnimal
{
    ...
    animal = [CCMenuItemImage itemFromNormalImage:[NSString stringWithFormat:@"%i.png", number] 
                                    selectedImage:[NSString stringWithFormat:@"%i.png", number]
                                           target:self 
...
-(void)tapAnimal
{
    ...
    if (animal.tag == kRoosterTag)
    {
        ...
        [self.guiLayer formatCocoString:countCoco];
        if (countCoco == 10)
        {
            for (int g = 0; g < 10; g++)
            {
            ...
            CCLOG(@"you win!");
            [self gameOver];
            [self.guiLayer ShowGameOverLabel];
